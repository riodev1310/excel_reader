...
if "Visitors data" not in wb.sheetnames:
    ...
    exit(1)
ws_source = wb["Visitors data"]

# Create a new sheet for processed data
ws_processed = wb.create_sheet(title="Processed data")

# Copy header row from Visitors data to Processed data
for col in range(1, ws_source.max_column + 1):
    ws_processed.cell(row=1, column=col).value = ws_source.cell(row=1, column=col).value
...
def insert_new_column(new_column_name, insert_after, ws):
...
# Copy data from Visitors data to Processed data
for row in range(2, ws_source.max_row + 1):
    for col in range(1, ws_source.max_column + 1):
        ws_processed.cell(row=row, column=col).value = ws_source.cell(row=row, column=col).value


# Insert new columns into Processed data sheet
...
for col in range(1, ws_processed.max_column + 1):
    header = ws_processed.cell(row=header_row, column=col).value
...
# Map the data for each row in Processed data
for row in range(2, ws_processed.max_row + 1):
    # Get ID values
    channel_id = ws_processed.cell(row=row, column=col_map['Channel id']).value
    product_id = ws_processed.cell(row=row, column=col_map['Product id']).value
    pricing_plan_id = ws_processed.cell(row=row, column=col_map['Pricing plan id']).value
    city_id = ws_processed.cell(row=row, column=col_map['City id']).value

    # Map IDs to values, with default for invalid IDs
    ws_processed.cell(row=row, column=col_map['Channel']).value = channel_map.get(channel_id, 'Unknown')
    ws_processed.cell(row=row, column=col_map['Products']).value = product_map.get(product_id, 'Unknown')
    ws_processed.cell(row=row, column=col_map['Pricing Plan']).value = pricing_plan_map.get(pricing_plan_id, 'Unknown')
    ws_processed.cell(row=row, column=col_map['City']).value = city_map.get(city_id, 'Unknown')

# Save the modified workbook
try:
    wb.save("company data test.xlsx")
    print("Data mapping completed and saved to company data test.xlsx with new 'Processed data' sheet")
